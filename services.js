...
function *loadRoutes () {
  try {
    const routes = yield models.Route.getJoin(false).run()
    return routes
  } catch (err) {
...
function *loadRoute (shortName) {
  try {
    const routes = yield models.Route
      .filter({ shortName })
      .getJoin({
        shapes: { _apply: seq => seq.limit(1) }, // TODO: get appropriate shape for the request time + schedule
        alerts: true
      })
      .run()
...
module.exports = { loadRoutes, loadRoute, loadVehiclesForRoute }
